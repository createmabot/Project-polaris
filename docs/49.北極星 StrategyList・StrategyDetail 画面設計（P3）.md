...
- `SymbolDetail` 側の Strategy Application 表示整理により、StrategyDetail / StrategyVersionDetail への参照導線は維持したまま application row の見通しを改善した。
- StrategyList / StrategyDetail の責務や route 方針は変更していない。

## 追記（2026-05-10）

- `BacktestDetail` 側の銘柄起点 application backlink 表示を整理した。
- `StrategyDetail` は strategy 定義の詳細、`BacktestDetail` は個別検証レポート詳細という役割分担は維持している。

## 追記（2026-05-10）

- StrategyList / StrategyDetail 周辺の P3 現在地は [[53.北極星 P3現在地と残課題整理（P3）]] に反映した。
- StrategyDetail の既存 data 表示、archive / restore、applied symbols / related reports read-only 表示は完了扱いとし、favorite / richer metadata は後続判断とする。
