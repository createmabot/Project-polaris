...
- Home 投資カレンダーは 1ヶ月 compact calendar grid と `前月` / `次月` の frontend local month navigation まで完了済み。後続候補は mobile での横スクロール体験改善、day cell の詳細 popover、provider / event type filter、表示月と refresh range の UX 整理であり、外部 calendar sync や notification は引き続き後続判断にする。
