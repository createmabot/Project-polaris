...
- Close internal backtest UI phase 1 では SymbolDetail / StrategyVersionDetail の新規実行 UI を閉じ、TradingView / Pine / CSV import を主導線に寄せた。backend route / worker / Prisma / DB / API contract は互換維持しており、internal backtest 実行系の削除・停止は後続判断にする。
- Internal backtest backend deprecation Stage 2B では、`/api/internal-backtests` route unregister / 410 Gone、application internal start / report conversion endpoint の 410 Gone / delete、`setupInternalBacktestWorker` startup stop を判断する。第一候補は 410 Gone と worker startup stop。queued / running execution の cancel / failed 扱いは運用判断を先に決める。
- Internal backtest backend deprecation Stage 2C では、data audit 後に internal tables / columns / relations の drop、service / queue / worker / tests の削除、legacy docs cleanup を判断する。`SymbolStrategyApplicationRun.internalBacktestExecutionId` を drop すると historical relation が失われるため、Backtest snapshot で十分かを確認する。
- 次段階の影響候補: `backend/src/index.ts`、`backend/src/routes/internal-backtests.ts`、`backend/src/routes/symbol-strategy-applications.ts`、`backend/src/queue/internal-backtests.ts`、`backend/src/internal-backtests/*`、`backend/src/backtests/ai-summary.ts`、`backend/prisma/schema.prisma`、`backend/prisma/migrations/*`、`backend/test/internal-backtests*.test.ts`、`backend/test/symbol-strategy-applications*.test.ts`、`frontend/src/pages/BacktestDetail.tsx`、`frontend/src/pages/ApplicationDetail.tsx`、`frontend/src/api/types.ts`、`docs/仕様書/*`、`docs/運用ドキュメント/*`、`docs/作業進捗管理/*`。
