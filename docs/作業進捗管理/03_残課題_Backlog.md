...
- Pine generation は LLM-first path へ移行済みである。deterministic generator は baseline / emergency fallback / test fixture として残し、LLM output は validation / bounded repair を通す。safe strategy coding prompt rule として、guarded `strategy.entry` / `strategy.exit`、entry-time ATR `var` pattern、entry block 内の `strategy.position_avg_price` stop 計算禁止、volume plot 原則なし、limitations の warnings / assumptions 明記を docs に固定済み。real local_llm 品質改善、TradingView compile automation、auto save / auto backtest / AI summary 自動連鎖は後続判断にする。
