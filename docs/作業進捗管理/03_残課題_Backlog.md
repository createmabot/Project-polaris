...
- 将来 `normalized_strategy_spec` を single source of truth にする場合は、Pine generation を spec-based に寄せる設計を別 phase で行う。現時点では natural language rule から Pine と spec を別々に生成し、ズレを診断するだけに留める。
