...
- CSV import report と internal backtest report の本格比較 UX を判断する。internal backtest report は signal / fill / condition debug 付き取引一覧 preview、TradingView List of Trades CSV との trade_no順診断比較まで表示済みだが、厳密な trade matching、export、本格 diff、TradingView parity 判定は後続判断とする。
...
- Normalized Strategy Spec v1 は foundation artifact として実装済みであり、Internal Backtest Engine v1 の明示実行 API / StrategyVersionDetail UI に接続済みである。後続では spec 表現力拡張、unsupported feature 分類精度、ユーザー編集 UI、spec diff、market data coverage と spec readiness の詳細照合、timeframe / market / position sizing 拡張、日本株100株単元対応を別 phase で検討する。現時点では spec 生成だけで internal backtest / Pine generation / optimization を自動起動しない。
...
Phase 3: BacktestDetail application report から improvement clone（実装済み）。

- SymbolDetail application row の直接 `改善版を作る` action は廃止し、BacktestDetail で検証レポートと AI総評を確認した後に改善版作成へ進む。
- 既存 `strategy version clone` flow を明示的に呼び、新 version の StrategyVersionDetail へ遷移する。
- `symbol_id` / `application_id` / `strategy_version_id` / `source_backtest_id` / `return` / `mode=improve_application` 相当の context を渡す。
- StrategyVersionDetail は元 backtest / 元 application / 元 version を示し、rule edit / Pine regenerate は既存導線を使う。
...
## Strategy Optimization Session MVP 後続課題

- candidate の自動 winner 判定、ranking、auto apply は今回対象外。実検証 report が揃った後の read-only 比較 UI を見てから別設計にする。
- optimization session の objective type は初期値 `balanced` を中心に扱う。risk / drawdown / trade count 優先などの objective preset は、評価軸と UI 表示方針を固めてから追加する。
- candidate 比較 board の集計、差分表示、report 比較の高度化は後続候補。raw CSV、raw import text、raw Pine、raw prompt、provider response は表示・保存しない。
- candidate から作成した version の lineage / annotation 表示は既存 StrategyVersion lineage と最小 annotation を利用する。replace flow、old application auto archive、自動 backtest は後続判断。

## Strategy implementation alignment 後続課題

- Pine semantic parser は common pattern の deterministic 診断に限定する。後続では `strategy.exit` の stop / limit / trail、複合 condition、stateful setup、`barssince` / holding bars、ATR / percent stop の variant、MACD histogram の変数名推定を fixture ベースで拡張する。
- alignment report は TradingView parity を保証しない。Internal backtest と TradingView の差分分析では、alignment status、約定モデル、価格データ、quantity rounding、commission / slippage を分けて扱う。
- `normalized_strategy_spec` を implementation contract に寄せる方針へ移行中。現在は Pine generation が spec available 時に spec-first prompt / payload を使い、spec missing 時は legacy natural-language-only generation を warning 付きで許可する foundation 段階。後続では Pine生成前の spec availability gating、`spec生成 -> Pine生成` の明示連続操作、alignment diagnostics を「spec -> Pine の実装確認」として再定義する。

## Market Data Foundation 後続課題

- external market data provider の追加、scheduled refresh、crawler、provider observation / freshness UI は別設計にする。required check に real provider / real external API を入れない。
- Internal Backtest Engine v1 後続では、保存済み `MarketPriceBar` の coverage warning 精度、adjusted price 方針、timeframe 拡張、market 別仕様、missing bar handling、corporate action adjustment、commission / slippage / sizing、application run 連携を別 phase で整理する。
- manual CSV import の header alias、timezone、adjusted OHLC、split / dividend adjustment、large file handling、import rollback / preview、duplicate source policy は利用実績を見て拡張する。
- batch symbol import、watchlist 単位 import、provider 管理 UI、user customization は後続判断にする。
- raw CSV / raw import text / external raw response / endpoint / model 実値 / secret / token / credential / local path / stack trace を保存・表示しない境界は維持する。
