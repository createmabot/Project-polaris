...
- Phase P1 / P3: market-level event provider。Alpha Vantage は初回 provider として実装済み。CPI / retail sales / unemployment / nonfarm payroll / real GDP は発表済み data series の observation date として扱い、IPO calendar は `ipo` event として扱う。PPI は Alpha Vantage 公式 docs 上で無料 API function を確認できていないため後続候補に残す。FRED / FMP は比較候補として残し、FOMC、日銀、JPX / NYSE / Nasdaq 休場日は無料 API がない場合だけ公式 source の限定 scraping または curated fixture を検討する。
...
- 未取得の高優先 market event: FOMC 日程、日銀金融政策決定会合、米国市場休場日 / 短縮取引日、PPI。公式 source parser / curated seed を検討するが、real external page access を required check に入れず、raw HTML / raw JSON を保存しない。
