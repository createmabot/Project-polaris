...
- Normalized Strategy Spec v1 は foundation artifact として実装済みである。後続では spec 表現力拡張、unsupported feature 分類精度、ユーザー編集 UI、spec diff、internal backtest engine への read-only 接続、market data coverage と spec readiness の照合を別 phase で検討する。現時点では spec 生成だけで internal backtest / Pine generation / optimization を自動起動しない。
...
- internal backtest engine 連携は、保存済み `MarketPriceBar` の coverage / adjusted price 方針、timeframe 拡張、market 別仕様、missing bar handling、corporate action adjustment を整理してから実装判断する。
