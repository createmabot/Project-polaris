...
- proposal history full management。StrategyLab の proposal history section で provider / status / selected / search / pagination による compact 管理導線まで完了済み。list response は sanitized summary に限定し、raw prompt / raw provider response / raw Codex output / endpoint / model 実値 / user_hint 全文 / candidate 自由文本文は返さない。
