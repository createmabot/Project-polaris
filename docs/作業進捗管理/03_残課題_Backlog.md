...
- Home 実運用 UI 改善では、Home top hero header と比較 / StrategyLab CTA を削除し、AIデイリーサマリー切り替えを section-local update にした。後続候補は summary section のより細かい skeleton 表示、直近切替結果の小さな cache、browser smoke での scroll 位置確認に限定する。
...
- Home 投資カレンダーは Home 上部への配置、1ヶ月 compact calendar grid、`前月` / `次月` の frontend local month navigation、title 中心の短い chip 表示まで完了済み。後続候補は mobile での横スクロール体験改善、day cell の詳細 popover、provider / event type filter、表示月と refresh range の UX 整理であり、外部 calendar sync や notification は引き続き後続判断にする。
