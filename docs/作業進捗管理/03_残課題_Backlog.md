...
- Codex CLI manual JSON import。`openai_api` provider ではなく、ユーザーが手動で生成した `strategy_proposal_candidates` JSON を StrategyLab に貼り付け / file text import する workflow として設計する。backend から Codex CLI を自動起動せず、raw Codex output を保存しない。複数候補 import、proposal history 保存、selection API 連携、StrategyLab 最小 UI を対象候補にする。
...
| 3 | LLM strategy proposal provider operations | benchmark result recording workflow と cost / rate guard hardening 完了後に openai_api、sanitized provider event log persistence、fallback metadata、benchmark result DB table、distributed rate limit / hard cost cap を個別設計してから段階判断する。 |
