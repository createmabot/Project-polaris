...
- Pine generation は LLM-first path へ移行済みである。deterministic generator は baseline / emergency fallback / test fixture として残し、LLM output は validation / bounded repair を通す。safe strategy coding prompt rule として、guarded `strategy.entry` / `strategy.exit`、position open 直後の entry-time ATR `var` capture、open から flat への transition reset、position open 中かつ entry-time state available の場合だけ stop price を計算する rule、actual entry price として position open 後の `strategy.position_avg_price` を stop loss 基準にする rule、`entry_price := close` の無断代替禁止、ATR stop は `strategy.exit(..., stop=...)` 優先、manual bar-based stop 明示時以外の `low <= stopLossPrice` + `strategy.close()` 回避、entry block 内の `strategy.position_avg_price` stop 計算禁止、percentage stop は position open 中の `strategy.position_avg_price` 基準で計算し ATR が明示されない限り `entryAtr` / `ta.atr` を混入させない rule、RSI / oscillator threshold direction 維持、setup -> trigger の state variable 利用、below wording の状態条件扱い、overlay price chart で oscillator plot を既定では出さない rule、`color.color.*` / `plot.style_dashed` 回避、stop loss plot の position / `na` guard、unused state 抑制、generated_script comment を短い section comment に限定する rule、volume plot 原則なし、limitations の warnings / assumptions 明記を docs に固定済み。real local_llm は manual smoke に限定し、required check は fake / deterministic tests を使う。TradingView compile automation、TradingView 自動保存、auto backtest / AI summary 自動連鎖は後続判断にする。
