...
- Phase P1: market-level event provider。Alpha Vantage は初回 provider として実装済み。CPI / retail sales / unemployment / nonfarm payroll は発表済み data series の observation date として扱い、IPO calendar は `ipo` event として扱う。FRED / FMP は比較候補として残し、FOMC、日銀、JPX / NYSE / Nasdaq 休場日は無料 API がない場合だけ公式 source の限定 scraping または curated fixture を検討する。
- Phase P2: JP stock symbol-level provider。J-Quants の無料 plan を第一候補として、決算発表予定、配当権利落ち、配当支払予定を SymbolDetail / watchlist / positions に接続する。無料枠で不足する event は paid plan へ進まず後続判断にする。
