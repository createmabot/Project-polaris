...
- Internal backtest backend deprecation Stage 2B では、`/api/internal-backtests`、application internal start / report conversion endpoint を HTTP 410 / `INTERNAL_BACKTEST_DEPRECATED` にし、`setupInternalBacktestWorker` startup を停止する。Prisma / DB / migration は keep し、historical internal report は read-only legacy として維持する。
- Internal backtest backend deprecation Stage 2C-1 では、read-only audit script と runbook で残存 execution / artifact / report / run / AI summary 関連 count と snapshot retention risk を確認する。既存 DB data は変更しない。
- Internal backtest backend deprecation Stage 2C では、audit 結果確認後に internal tables / columns / relations の drop、service / queue / worker / tests の削除、legacy docs cleanup を判断する。`SymbolStrategyApplicationRun.internalBacktestExecutionId` を drop すると historical relation が失われるため、Backtest snapshot で十分かを確認する。
- Stage 2C 影響候補: `backend/src/routes/internal-backtests.ts`、`backend/src/routes/symbol-strategy-applications.ts`、`backend/src/queue/internal-backtests.ts`、`backend/src/internal-backtests/*`、`backend/src/backtests/ai-summary.ts`、`backend/prisma/schema.prisma`、`backend/prisma/migrations/*`、`backend/test/internal-backtests*.test.ts`、`backend/test/symbol-strategy-applications*.test.ts`、`frontend/src/pages/BacktestDetail.tsx`、`frontend/src/pages/ApplicationDetail.tsx`、`frontend/src/api/types.ts`、`docs/仕様書/*`、`docs/運用ドキュメント/*`、`docs/作業進捗管理/*`。
...
- artifact metadata / retention / file access boundary の設計方針と UI path 非表示は完了済み。Stage 2B 以降、internal_backtests engine_actual trades / equity JSON read endpoint は 410 であり、historical report では BacktestDetail の artifact pointer read-only 表示に限定する。
