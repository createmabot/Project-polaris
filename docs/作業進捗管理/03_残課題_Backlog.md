...
- Pine generation は LLM-first path へ移行済みである。deterministic generator は baseline / emergency fallback / test fixture として残し、LLM output は validation / bounded repair を通す。real local_llm 品質改善、prompt tuning、TradingView compile automation、auto save / auto backtest / AI summary 自動連鎖は後続判断にする。
