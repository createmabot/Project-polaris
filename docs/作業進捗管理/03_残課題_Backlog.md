...
- Pine generation は LLM-first path へ移行済みである。deterministic generator は baseline / emergency fallback / test fixture として残し、LLM output は validation / bounded repair を通す。safe strategy coding prompt rule として、guarded `strategy.entry` / `strategy.exit`、entry-time ATR `var` pattern、open から flat への transition reset、position open 中かつ entry-time state available の場合だけ stop price を計算する rule、entry block 内の `strategy.position_avg_price` stop 計算禁止、volume plot 原則なし、limitations の warnings / assumptions 明記を docs に固定済み。real local_llm は manual smoke に限定し、TradingView compile automation、auto save / auto backtest / AI summary 自動連鎖は後続判断にする。
