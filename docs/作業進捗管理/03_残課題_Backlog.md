...
- sanitized provider event log persistence は現在 phase 対象。初回は provider call / manual import / retry / rate limit / validation failure の sanitized event persistence とし、raw prompt / raw response / raw Codex output / endpoint / model 実値 / user_hint 全文 / candidate 自由文本文は保存しない。
