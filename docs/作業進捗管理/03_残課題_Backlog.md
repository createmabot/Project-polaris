...
- proposal history soft archive は現在 phase 対象。初回は `StrategyProposalRun.archivedAt` による通常一覧からの非表示、archived filter、archive / unarchive action、archived detail / selection 許可までに限定し、hard delete / retention job / export は後続判断にする。
