...
- public provider の source reliability 評価は `docs/運用ドキュメント/13_Investment_calendar_provider調査.md` に整理済み。次に実装する場合は、Option A の market-level event provider を第一候補にする。
- Phase P1: market-level event provider。対象は FOMC、日銀、米 CPI、米雇用統計、GDP、PPI、小売売上高、ISM / PMI、JPX / NYSE / Nasdaq 休場日。Home の investment calendar を先に実用化する。
- Phase P2: JP stock symbol-level provider。J-Quants を第一候補として、決算発表予定、配当権利落ち、配当支払予定を SymbolDetail / watchlist / positions に接続する。
- Phase P3: disclosure / filing enrichment。TDnet API は paid optional、EDINET は submitted document event として扱う。
