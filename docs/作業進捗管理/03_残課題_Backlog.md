...
- Pine generation の初回 market / timeframe 拡張は `JP_STOCK|US_STOCK` と `D|1D|4H|1H` まで完了済みである。internal backtest engine の市場 / 時間足対応拡張、market data provider 拡張、FX / CRYPTO、15M / 30M、TradingView compile 自動実行は後続判断にする。
