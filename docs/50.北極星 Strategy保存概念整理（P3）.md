...
- Symbol Strategy Application の第一段階として、read API に続いて POST 保存 API を追加した。
- application parent / run table 方針は維持し、今回の POST API では run / Backtest / BacktestImport / InternalBacktestExecution は作成しない。
- BacktestDetail は引き続き個別検証レポート詳細として維持する。

## 追記（2026-05-10 その9）

- Symbol Strategy Application の保存 UI を frontend に接続した。
- application parent / run table 方針は維持し、今回は application parent の保存と一覧表示までに限定した。
- run 作成、CSV取込接続、内部バックテスト接続は未実装である。

## 追記（2026-05-10 その10）

- SymbolStrategyApplicationRun が `csv_import` として作成され、Backtest / BacktestImport へ link する方針を実装に反映した。
- application parent と Backtest report detail の分離は維持する。
- internal backtest run、application archive / restore、related reports / applied symbols display は未実装である。

## 追記（2026-05-10 その11）

- SymbolStrategyApplicationRun が `internal_backtest` としても作成され、InternalBacktestExecution へ link するようになった。
- CSV import run と internal backtest run は application 配下の実行単位として扱い、BacktestDetail は引き続き report detail として維持する。
- internal backtest から Backtest report へ変換する処理、BacktestDetail backlink、related reports / applied symbols display は未実装である。

## 追記（2026-05-10 その12）

- Symbol Strategy Application parent / run / BacktestReport link の read-only 表示を進めた。
- `StrategyDetail` では applied symbols / related reports、`BacktestDetail` では application backlink を表示する。
- application archive / restore、internal execution result detail、report 化の拡張は後続判断とする。

## 追記（2026-05-10 その13）

- Symbol Strategy Application でも hard delete ではなく archive / restore を採用した。
- archive / restore は application status の更新であり、application run / Backtest report link は維持する。
- metadata table や hard delete は追加していない。

## 追記（2026-05-10 その9）

- `SymbolStrategyApplicationRun` が `internal_backtest` の場合、link された `InternalBacktestExecution` の status / result summary を `SymbolDetail` で read-only 表示できるようにした。
- application parent / run / execution の関係を壊さず、保存概念や DB schema は変更していない。
- Backtest report への変換、AI summary generation、`BacktestDetail` redesign は未実装のままである。

## 追記（2026-05-10 その14）

- Backtest report から Symbol Strategy Application parent / run への read-only backlink 表示を整理した。
- application / run / report の保存概念は維持し、BacktestDetail では親概念への参照情報だけを表示する。
- DB schema や保存方針は変更していない。
