...
- `StrategyRule` は `id`、`userId`、`title`、`status`、`createdAt`、`updatedAt` を持ち、`StrategyRuleVersion` と relation を持つ。
...
- `StrategyRuleVersion` は `strategyRuleId`、`versionNo`、`sourcePrompt`、`normalizedRuleJson`、`generatedPine`、`pineVersion`、`generationStatus`、`validationStatus`、`aiModelName`、`createdAt`、`updatedAt` を持つ。
...
- `Backtest` は `strategyRuleVersionId`、`strategySnapshotJson`、`title`、`executionSource`、`market`、`timeframe`、`status`、`startedAt`、`finishedAt`、`metricsJson`、`tradesJson`、`artifactUrl`、`createdAt`、`updatedAt` を持つ。
...
- `Symbol` は `symbol`、`symbolCode`、`tradingviewSymbol`、`marketCode`、`displayName`、`currencyCode`、`assetType`、`isActive` などを持つ。
