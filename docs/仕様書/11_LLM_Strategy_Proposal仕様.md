...
- repair に渡す reviewer issue は priority mapping で制御する。severity `error`、`repairable=true`、priority > 0 の issue だけを code で重複排除し、priority 順の最大 3 件を `repair_request.reviewIssues` に入れる。priority 0 の readability / plotting preference / below-vs-crossunder nuance / minor unused variable / narrative comment は repair を起動しない。warning-only review は成功として扱う。
- `repair_request` がある場合、provider には通常生成用の大きな prompt ではなく repair 専用 prompt を使い、listed issue だけを直し、無関係な strategy logic / intent を保持する。warnings / assumptions は日本語、`generated_script` は Pine 本文のみとし、raw prompt / response / endpoint / model / secret / local path / stack trace は出さない。
- selected repair issue には internal prompt 用の `repair_template` を付与できる。template は `entry_guard_risk`、`entry_time_atr_not_persisted`、`stop_order_guard_risk`、`setup_trigger_state_risk`、ADX / DMI unsafe pattern、Donchian current-bar self-reference、unsupported function alias などの targeted repair に使い、外部 API / DB schema は変更しない。repair attempt が 2 回目以降の場合は template を優先して同一 issue の再発修正を強める。
...
- semantic hardening では、setup state variable 名が `setupActive` 以外（例: `priceWasBelowBB` / `belowBand` / `touchedLower` / `wasBelow` / `pullbackActive`）でも premature reset / entry 後 reset missing を `setup_trigger_state_risk` として扱う。Donchian channel は current bar の `ta.highest(high, len)` / `ta.lowest(low, len)` ではなく prior channel（例: `upperBand[1]` / `lowerBand[1]`）との比較を要求し、違反は `donchian_current_bar_self_reference` とする。entry-time ATR stop は position-open transition で persisted `entryAtr` を使う必要があり、current ATR 直接利用は `entry_time_atr_not_persisted` とする。
