...
- Pine generation の warnings / assumptions は deterministic generator、provider output、post-processing の既知英文を UI 表示向けに日本語化する。未知の provider 文言は自動翻訳せず、そのまま保持して後続 mapping 追加で対応する。
