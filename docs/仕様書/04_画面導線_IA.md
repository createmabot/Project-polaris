...
Home は watchlist / positions 対象銘柄と market-level event をまとめた投資カレンダーを表示する。SymbolDetail は対象銘柄の投資カレンダーを表示し、手動更新ボタンを持つ。TradingView chart widget は SymbolDetail から削除し、TradingView は外部での chart / Pine 検証と CSV import の主導線として扱う。
