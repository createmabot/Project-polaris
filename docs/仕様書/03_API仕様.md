...
`INVESTMENT_CALENDAR_PROVIDER=official_market` を設定した場合、Home の manual refresh は FOMC、日銀金融政策決定会合、米国市場休場日 / 短縮取引日を market-level event として保存できる。FOMC / BOJ は `central_bank`、US holiday / early close は `market_holiday` として扱う。default は bundled curated fixture event を使い、任意の official source URL が設定された場合だけ manual refresh 時に fetch する。required test は fixture HTML / fixture JSON / mocked fetch のみで、real website access は使わない。raw HTML / raw JSON / endpoint 実値 / stack trace は API response に含めない。
