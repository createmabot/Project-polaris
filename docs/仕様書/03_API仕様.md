...
Internal Backtest Engine v1:

- `POST /api/strategy-versions/:versionId/internal-backtests`
  ...
  - raw CSV、raw import text、raw Pine、raw prompt、provider response、endpoint、model 実値、secret、token、credential、local path、stack trace は response / snapshot に含めない。
- `/api/internal-backtests/*` route、application 起点 internal start / report conversion endpoint、internal execution / artifact / data source event tables、worker、queue、service、audit command は Stage 2C cleanup 後も復活させない。
- historical internal report と v1 report はどちらも `Backtest.strategySnapshotJson` の `result_summary` / `artifact_pointer` / `internal_backtest_execution_id` snapshot で説明する。DB execution relation は参照しない。
...
- `GET /api/symbols/:symbolId/market-data/coverage`
  ...
  - `meta.internal_backtest_ready` は market data 単体では実行可否を確定しないため、coverage の read metadata として扱う。内部バックテスト実行は StrategyVersion の normalized spec と symbol price bars の組み合わせで判定する。
...
Market data API は raw CSV、raw import text、external provider raw response、endpoint、model 実値、secret、token、credential、local path、stack trace を response に含めない。external market data provider、scheduled refresh、crawler は今回追加しない。
...
- `GET /api/strategy-versions/:versionId/normalized-spec` は、保存済み `normalizedRuleJson` が `normalized_strategy_spec` v1 の場合に `status=available` として返す。未生成または無効化済みの場合は `status=unavailable`、`normalized_spec=null` を返す。meta の `internal_backtest_ready` は schema v1 / `D` / `long_only` を満たす場合に true とし、最終的な unsupported feature / market data validation は internal backtest 実行 API 側で行う。
- `POST /api/strategy-versions/:versionId/normalized-spec/generate` は、保存済み natural language rule / market / timeframe から LLM-first で `normalized_strategy_spec` v1 を生成し、deterministic validator / sanitizer を通して既存 `StrategyRuleVersion.normalizedRuleJson` に保存する。`STRATEGY_SPEC_PROVIDER=local_llm|deterministic|openai_api` で provider を選び、既定は `local_llm` とする。provider unavailable / invalid output の場合は deterministic parser に fallback し、`source.fallback_used=true` と warning を返す。response は最小の strategy version meta、`normalized_spec`、warnings、assumptions を返し、raw natural language rule、raw Pine、raw prompt、provider response、endpoint、model 実値、secret、token、local path、stack trace は返さない。実行だけでは Pine generation、backtest、AI summary、optimization、application apply を起動しない。
- `GET /api/strategy-versions/:versionId/implementation-alignment` は、保存済み generated Pine と `normalized_strategy_spec` v1 から read-only の `strategy_implementation_alignment` v1 report を計算して返す。entry / exit / risk / filters / indicators の common pattern を deterministic に比較し、`ok|warning|mismatch|unavailable`、matched / mismatches / missing_in_pine / missing_in_spec を返す。report は保存しない。実行だけでは Pine generation、spec generation、backtest、AI summary、optimization、application apply を起動しない。raw prompt、provider response、endpoint、model 実値、secret、token、local path、stack trace は返さない。generated Pine の全文 expose は既存 Pine 表示 API の範囲に限定し、alignment response では semantic label のみにする。
