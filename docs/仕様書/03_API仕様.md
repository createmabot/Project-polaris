...
- `GET /api/strategy-versions/:versionId/normalized-spec` は、保存済み `normalizedRuleJson` が `normalized_strategy_spec` v1 の場合に `status=available` として返す。未生成または無効化済みの場合は `status=unavailable`、`normalized_spec=null` を返す。meta は `internal_backtest_ready=false` を返し、internal backtest 実行可能とは扱わない。
