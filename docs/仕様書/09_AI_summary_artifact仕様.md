...
Artifact metadata / retention policy phase は、metadata 正本仕様、path 系 metadata 非表示、raw JSON sanitization、ApplicationDetail の artifact path 非表示 note まで完了扱いである。Stage 2C cleanup 後は internal backtest artifact read endpoint / table は削除済みで、historical report の artifact pointer は Backtest snapshot metadata としてのみ扱う。
...
- `backtest_review_summary` は `schema_version=1.0` を維持する。Backtest AI summary の本文は、改善ループに接続できるように、概要、主要メトリクス、成績評価、問題の切り分け、改善仮説、自然言語ルール改善案、Pine修正依頼に入れるべきではない注意、次に試す検証案、注意点を含む。低い trade count、低い Profit Factor、低い win rate、大きい max drawdown、悪い net profit は、entry / exit / risk management / market regime / stop loss / position management / time exit の見直し仮説に接続する。
- `payload.next_actions` は具体的な改善・再検証 action、`payload.overall_view` は StrategyVersionDetail の自然言語ルール改善に転用しやすい改善案として扱う。`payload.rule_refinement_candidates` は v1.0 互換の optional field として、entry / exit / risk / filter などの自然言語ルール本文に反映しやすい候補を格納できる。候補には title、target area、rationale、change summary、entry / exit / risk の具体変更、validation plan、期待される metric 影響を safe text で持たせる。
- Pine修正再生成に使う `revision_request` は、compile error / validation note / TradingView 上の挙動調整に限定する。AI summary 生成だけで strategy clone、Pine generation、backtest、application apply は起動しない。CSV全文、取込本文、raw prompt、provider response、generated Pine 全文は summary input / output に含めない。
- StrategyVersionDetail の LLM rewrite draft は、保存済み AI summary の `payload.rule_refinement_candidates` を優先し、なければ `payload.next_actions` / `overall_view` / `risks` / `strengths` と主要 metrics を、自然言語ルール本文の再構成 context として扱う。AI summary 本文や改善履歴を natural_language_rule に追記せず、単一の最新ルール本文 draft を作る。rewrite draft 実行だけで AI summary 再生成、Pine generation、backtest、application apply は起動しない。

## 3. auto enqueue 現行範囲

- CSV import report: direct CSV import route と application 起点 CSV import route の `parse_status=parsed` 直後に auto enqueue する。
- historical internal backtest report: Stage 2C cleanup 後は new conversion endpoint が存在しないため、conversion 起点 auto enqueue は新規には発生しない。既存 internal backtest report の AI summary 表示と、generic Backtest manual generation は維持する。
...
phase 1 は PR #319 / #320 / #332 により、CSV import parsed report、application 起点 CSV import、当時の internal backtest report conversion の auto enqueue と read-only visibility 整理まで完了扱いであった。Stage 2C cleanup 後は CSV import parsed report の auto enqueue と Backtest 汎用 manual generation を現行対象とする。

### 3-1. quality / cost operations boundary

AI summary の quality / cost operations は、自動生成対象を広げる前に運用境界を固定するための設計整理である。

- 現行 auto enqueue は CSV import parsed report に限定する。internal backtest report conversion endpoint は削除済みのため新規 auto enqueue は発生しない。
...
Stage 2C 以降の internal backtest report は historical legacy として扱う。新規生成・変換・artifact read path は存在せず、既存 snapshot と既存 summary を読めることを優先する。
...
`artifact_pointer` は、artifact 本体ではなく、historical internal report snapshot に残る参照 metadata である。現行画面では metadata summary と raw JSON を read-only に表示するが、BacktestDetail では artifact file content の読込、download、diff は行わない。
...
- 保存済み `artifact_pointer` metadata は historical report snapshot の read-only context として扱う。
...
Stage 2C cleanup 後、現行の artifact file access endpoint は存在しない。BacktestDetail は historical snapshot に残る artifact pointer metadata の read-only 表示だけを担当する。新規 download endpoint、file token、signed URL、backend proxy の本格実装は見送る。

現行で維持する範囲:

- Backtest snapshot 内の artifact pointer metadata を summary / raw JSON として表示する。
- stored artifact file existence は保証しない。
- artifact path から UI link や file access route を生成しない。

境界:

- frontend に raw path、local path、absolute path を渡さない。
- UI link は execution ID や artifact path から導かない。
