...
- Internal backtest backend deprecation Stage 2B で report conversion endpoint を閉じた場合、internal backtest report conversion 起点の auto enqueue は新規には発生しない。既存 internal backtest report の AI summary 表示と、generic Backtest manual generation は維持する。
...
Stage 2A 以降の internal backtest report は historical legacy として扱う。新規生成・変換・artifact read path の拡張ではなく、既存 snapshot と既存 summary を読めることを優先する。
