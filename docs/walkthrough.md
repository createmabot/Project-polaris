...
## �ǉ�: webhook �N�_ E2E �e�X�g (2026-03-20)
- �ǉ��e�X�g: `backend/test/webhook-route.e2e.test.ts`
- �ړI: `POST /api/integrations/tradingview/webhook` ��N�_�ɁA`alert_event -> ai_job -> collect -> summary -> ai_summaries` �̏c������p���ۏ؂���
- ��Ȍ���:
  - ����n�� `structured_json.payload.reference_ids` ���ۑ������
  - `unresolved_symbol` / `needs_review` �� `ai_job` ��N�[���Ȃ�
  - collect �S���s�ł� summary ���i�� `insufficient_context=true`
  - duplicate webhook �� `duplicate_ignored` �� DB �s�ϐ�
  - partial failure �����ł� summary ����
- ���s�R�}���h:
  - `cd backend && npm run test:e2e:webhook-summary`
  - `cd backend && npm run test:e2e:collect-summary`

## 追加: compare 機能 MVP (2026-03-21)
- 追加 API:
  - `POST /api/comparisons` (2〜4 銘柄の比較セッション作成)
  - `GET /api/comparisons/:comparisonId` (銘柄ごとの比較表示データ取得)
- 追加 UI:
  - `/compare` (比較対象入力)
  - `/comparisons/:comparisonId` (比較表示)
- 取得項目:
  - symbol 基本情報
  - latest_ai_thesis_summary
  - latest_active_note
  - recent_alerts (最大3件/銘柄)
  - related_references (最大5件/銘柄)
  - latest_processing_status
- 導線:
  - Home -> Compare
  - Symbol Detail -> Compare
  - Alert Detail -> Compare
- build:
...
## 追加: compare generate integration/E2E テスト (2026-03-21)
- 追加テスト: `backend/test/comparison-generate.e2e.test.ts`
- 目的: `POST /api/comparisons/:comparisonId/generate` 起点で、`comparison_results` 保存と `GET /api/comparisons/:comparisonId` の `latest_result` 反映を継続保証する
- 主な検証:
  - 正常系: comparison 作成 -> generate 成功 -> comparison_results 保存
  - latest_result が detail API へ反映
  - symbol 数不足の validation error
  - 不正 comparisonId の 404
  - `structured_json` と `comparedMetricJson` 保存
  - 再生成時に最新1件を更新（MVP方針）
- 実行コマンド:
