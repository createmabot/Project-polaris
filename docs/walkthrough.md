...
Home 上部で `投資カレンダー` section を確認し、監視・保有銘柄の予定と市場イベントが 1ヶ月分の compact calendar grid で表示されることを確認する。`前月` / `次月` で表示月だけが切り替わり、Home 全体の再取得や provider refresh が起きないことを確認する。日付 cell には title 中心の短い event chip が積まれ、provider / source / fetched_at / importance text などの補助 metadata が chip 内に出ないこと、同日に event が多い場合は `+N件` として省略されること、土日 cell が平日と控えめに区別されること、provider / fetched_at / stale / partial_success の情報が grid 下部にまとまることを確認する。必要に応じて `投資カレンダーを更新` を押し、失敗時も Home 全体が壊れないことを確認する。
