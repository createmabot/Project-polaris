...
- Stage 2B 以降は backend route も HTTP 410 / `INTERNAL_BACKTEST_DEPRECATED` を返す。DB / Prisma は維持し、Stage 2C で DB / Prisma / tests cleanup を判断する。
...
- Stage 2B 以降は API 410 Gone と worker startup stop を backend tests / build で確認する。
...
- Stage 2B 以降は report conversion endpoint が HTTP 410 / `INTERNAL_BACKTEST_DEPRECATED` を返すため、UI からも API からも新規 conversion へ誘導しない。
