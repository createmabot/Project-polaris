...
4. invalid output があった場合は `repair_attempts` と `invalid_reason_codes` が sanitized metadata として確認できることを確認する。
5. TradingView compile は自動実行されないことを確認する。compile error を反映する場合は、TradingView で確認した内容を Pine 修正再生成の `compile_error_text` / `validation_note` / `revision_request` に手動入力する。
6. Pine generation / regeneration 後に Strategy 保存、backtest、AI summary が自動起動しないことを確認する。
7. UI / API response / screenshot / docs に raw prompt、raw provider response、endpoint、model 実値、secret、local path、stack trace が出ないことを確認する。
8. real `local_llm` 実体依存確認は manual optional とし、required check の前提にしない。
