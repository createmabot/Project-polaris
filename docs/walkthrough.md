...
- `SymbolDetail` は最小移行済みで、上部は `銘柄概要` に銘柄 identity と現在 snapshot をまとめ、`最新アラート` `最新AI論点カード` `Research Note` `関連参照情報` の section を整理し、`SideRail` と共存する画面構造にした。
