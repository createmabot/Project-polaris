...
Home で `投資カレンダー` section を確認し、監視・保有銘柄の予定と市場イベントが月単位の compact calendar grid で表示されることを確認する。日付 cell に event chip が積まれ、同日に event が多い場合は `+N件` として省略されることを確認する。必要に応じて `投資カレンダーを更新` を押し、失敗時も Home 全体が壊れないことを確認する。
