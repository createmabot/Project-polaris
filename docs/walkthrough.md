...
6. SymbolDetail の `関連参照情報を再取得` はユーザー操作起点でだけ実行し、実行中はボタンが disabled になり、成功または失敗の表示が出ることを確認する。reference card は日本語 type badge、公開日、plain text の title / summary を表示し、raw enum bracket、HTML fragment、raw URL 本文を出さないことも確認する
