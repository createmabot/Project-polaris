...
- Stage 2A は docs-only の deprecation design であり、backend route / DB / API contract はまだ互換維持する。Stage 2B では 410 Gone / worker startup stop、Stage 2C では DB / Prisma / tests cleanup を判断する。
...
- Stage 2B 以降は API 410 Gone や worker startup stop の確認へ移す。
...
- Stage 2A では report conversion endpoint をまだ変更しないが、UI からは新規 conversion へ誘導しない。
- Stage 2B では `POST /api/symbol-strategy-applications/:applicationId/internal-backtests/:executionId/report` を 410 Gone または削除するか判断する。第一候補は 410 Gone。
...
- Stage 2A 以降は SymbolDetail から succeeded internal execution を Backtest report 化しない。
...
4. internal backtest report conversion auto enqueue は legacy 完了範囲として扱う。Stage 2B で conversion endpoint を閉じた後は新規には発生しない。
