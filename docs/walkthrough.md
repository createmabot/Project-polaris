...
6. SymbolDetail の `関連参照情報を再取得` はユーザー操作起点でだけ実行し、実行中はボタンが disabled になり、成功または失敗の表示が出ることを確認する。reference card は日本語 type badge、公開日、plain text の title / summary を表示し、title と重複する summary 本文、raw enum bracket、HTML fragment、raw URL 本文を出さないことも確認する
...
- `SymbolDetail` は最小移行済みで、上部は `銘柄概要` に銘柄 identity と現在 snapshot をまとめ、`投資カレンダー`、`最新AI論点カード`、`最新アラート`、`ストラテジー / 検証結果`、`関連参照情報`、`Research Note` の順で日次確認から深掘りへ進む画面構造にした。
...
- `SymbolDetail` から `ストラテジー / 検証結果` 領域を起点に、CSV 取込、検証レポート詳細、銘柄別比較へ接続する。internal backtest の新規実行導線は閉じ、既存 internal report は read-only legacy として扱う。

## 31. SymbolDetail の `ストラテジー / 検証結果` section（2026-05-09 追加）

- `SymbolDetail` には `ストラテジー / 検証結果` section が表示される。
- 現行では保存済み application、compact filter、CSV 取込、run / report 履歴、BacktestDetail link が接続済みである。
- internal backtest 新規実行は主導線に戻さない。
- 次フェーズでは `この銘柄でストラテジー提案` から StrategyLab へ symbol context を渡す導線を追加するが、SymbolDetail 表示だけで proposal generation は起動しない。
...
- version を選んで `適用を保存` を押すと、既存 symbol strategy application 作成 API でこの銘柄へ適用できることを確認する。
- CSV 取込は保存済み application row から実行できることを確認する。internal backtest の新規実行 CTA は表示しない。
...
- `SymbolDetail` からの保存 UI 接続、CSV取込接続は段階実装対象である。internal backtest 接続は閉じた導線として扱う。
...
- 未保存の選択中 state では CSV取込が disabled であり、保存済み application の CSV取込は次項で確認する。internal backtest CTA は表示しない。
...
- internal backtest の新規実行 UI は Phase 1 で閉じていることを確認する。既存 internal report の read-only 表示と BacktestDetail link は維持する。

## 47. Symbol Strategy Application internal backtest wiring

- Phase 1 以降は `SymbolDetail` の保存済み application 行から内部バックテストを開始しない。
- 開始日 / 終了日、`engine_estimated` 表示、内部バックテスト開始ボタン、execution id / status / result / report conversion UI が表示されないことを確認する。
- Stage 2C cleanup 後は backend route / worker / service / execution relation / audit command は削除済み。新規実行は UI / API ともに扱わない。
...
- historical internal backtest run / report がある場合は read-only legacy として扱い、新規実行や conversion へ誘導しないことを確認する。
- `BacktestDetail` で銘柄起点 application backlink が表示されることを確認する。
- application archive / restore、BacktestDetail read-only 表示は既存導線として確認する。
...
- `SymbolDetail` の保存済み application row で latest run が `internal_backtest` の場合も、新規 polling / result fetch / conversion UI は表示しない。
- historical internal report が存在する場合は BacktestDetail link から read-only に確認する。
- Stage 2C cleanup 後は internal backtest API / worker / service / audit command が存在しない。historical internal report は Backtest snapshot ベースの read-only 表示だけを確認する。
...
## 50-1. Strategy Optimization Session MVP

- `BacktestDetail` の `AI 総評` に `rule_refinement_candidates` がある場合、`改善候補` section に候補番号、target、change、entry / exit / risk、validation plan が表示されることを確認する。
- `改善候補` の表示だけでは session 作成、clone、rewrite、Pine生成、backtest、apply が起動しないことを確認する。
- `この候補で改善版を作る` を明示クリックした場合だけ session 作成 / 再利用と candidate create-version が実行され、StrategyVersionDetail に symbol / application / source backtest / `refinement_candidate_id` 付きの improvement context URL で遷移することを確認する。
- StrategyVersionDetail の `検証結果からの改善メモ` に `選択中の改善候補` が表示され、`LLMで新しいルール本文を作る` が candidate context を優先することを確認する。表示だけでは rewrite を起動しない。
- `/strategy-optimization-sessions/:sessionId` では source backtest、candidate、created version、latest report metrics、base との差分を確認できる。勝者判定、自動 apply、自動 Pine generation は行わない。
...
- `SymbolDetail` の `ストラテジー / 検証結果` section で、保存済み application の summary と compact な `検証サマリー` が表示されることを確認する。
- `検証サマリー` は最新の読める検証結果と BacktestDetail link を中心に表示し、`latest_run` の詳細や CSV / internal source 別 latest pair を常時表示しないことを確認する。詳細な run / report 履歴は ApplicationDetail、個別 report detail は BacktestDetail へ進む導線として確認する。
- StrategyDetail / StrategyVersionDetail / BacktestDetail へのリンクが維持されていることを確認する。
- CSV import は必要時に展開する panel として表示され、CSVファイル、CSVテキスト、CSV取込の既存操作と archive / restore、historical internal report link が表示整理後も壊れていないことを確認する。internal backtest 新規実行は扱わない。
...
- 次に実装へ進む場合は、application / run 表示改善、UI component cleanup 第二段階、P3 smoke 追加範囲整理から選ぶ。internal backtest result の新規 Backtest report 化は削除済み導線である。
...
- internal backtest result の Backtest report 化は historical behavior として扱う。
- Stage 2C cleanup 後は report conversion endpoint 自体が存在しないため、UI からも API からも新規 conversion へ誘導しない。
- CSV import 由来 report と異なり、historical internal backtest report では `BacktestImport` を作らない。

## 追記（2026-05-10）

- `SymbolDetail` の saved application では internal backtest report conversion CTA を表示しない。
- historical report がある場合は BacktestDetail への導線だけを確認する。
- CSV import 由来 report の導線は従来どおり確認する。
...
- Stage 2B 以降は SymbolDetail から succeeded internal execution を Backtest report 化せず、API も 410 を返す。
- 既存 importless report がある場合だけ、BacktestDetail で backlink / AI summary context / artifact pointer 表示を read-only に確認する。
...
- BacktestDetail で internal backtest report section、BacktestImport なし説明、execution id、result summary、artifact pointer、SymbolDetail / StrategyDetail backlink が表示されることを確認する。`主要指標` には `strategySnapshotJson.result_summary.metrics` / `period` / `trade_period` 由来の総取引数、勝率、Profit Factor、最大ドローダウン、純利益、総リターン率、検証データ期間、取引発生期間が表示されることを確認する。trade がない場合は取引発生期間が `取引なし` として読めることを確認する。internal backtest report では取引一覧 preview が表示され、CSV import report では取引明細が未保存である説明が表示されることを確認する。
...
- `SymbolDetail` の saved application row では CSV / internal source 別 latest pair を常時前面に出さず、internal report がある場合は read-only legacy の短い補足に留めることを確認する。
...
- 通常確認では `StrategyDetail` の report type / source、`BacktestDetail` の同一 application 関連 report と metrics 横並び比較補助を確認する。`SymbolDetail` は最新の読める検証結果を中心に表示し、CSV / internal latest pair の詳細は常時表示しない。
...
- `SymbolDetail` の saved application filter で `レポート`、`すべて`、`reportあり`、`reportなし` が compact 表示されることを確認する。
...
- `SymbolDetail` の saved application row で application summary、最新の読める検証結果、表示 filter、件数 summary、run / report 履歴導線が確認できることを確認する。latest run と CSV / internal latest pair の詳細は常時表示しない。
...
- latest report、CSV import、archive、保存処理の挙動は変更しない。latest run と CSV / internal latest pair の詳細は SymbolDetail の常時表示から外し、run / report 履歴は ApplicationDetail で確認する。
...
- `reportあり` / `reportなし` との組み合わせ、latest report、pagination meta は維持する。latest run と CSV / internal latest pair は API response と filter 意味を維持しつつ、SymbolDetail の常時表示では前面に出さない。
...
- `SymbolDetail` の saved application filter で `run` と `run status` が compact 表示されることを確認する。
- run は `すべて` / `CSV` / `internal`、run status は `すべて` / `running` / `succeeded` / `failed` を最小 UI として確認する。
...
- `SymbolDetail` の saved application filter で report 有無、source、run、run status を切り替えられることを確認する。
...
- summary は status 対象を含めて表示され、archived application row でも application summary、最新の読める検証結果、run / report 履歴導線、internal legacy 補足が崩れないことを確認する。
...
- `SymbolDetail` の saved application filter で `strategy` / `version` text input が compact 表示されることを確認する。
...
- `SymbolDetail` では application 概要、最新の読める検証結果、Application Detail 入口を確認する。latest run と CSV / internal latest pair の詳細は Application Detail / BacktestDetail 側で確認する。
...
- Pine 状態、lineage、warnings / assumptions、generated pine が section / key-value / status 表示として読め、基本情報は折りたたみで確認できることを確認する。
...
- StrategyVersionDetail の自然言語ルール編集、Pine 修正再生成入力が既存導線を保ったまま表示され、forward validation note UI は表示されないことを確認する。
...
4. 同一 application 内に CSV import report と internal backtest report の両方の正規化済み取引明細がある場合は、`取引明細比較` が表示されることを確認する。比較は trade_no / 表示順の診断補助であり、厳密な約定照合ではない。
5. `ApplicationDetail` の report履歴 row から BacktestDetail comparison helper へ進む説明 / link label を確認する。
6. `BacktestComparisonDetail` は保存済み pairwise comparison の再訪画面であり、本格比較画面化は後続候補であることを確認する。
7. 確認では API / backend / frontend / DB / Prisma schema / test / 実行系操作を変更しない。
...
4. internal backtest report conversion auto enqueue は legacy 完了範囲として扱う。Stage 2B 以降は新規には発生しない。
...
7. 既存 manual generation endpoint / button、CSV import、historical internal report read-only 表示、DB schema は維持する。internal backtest 実行 / report conversion UI は主導線に戻さない。
...
## 46-1. SymbolDetail 起点の銘柄調査付き Codex CLI prompt smoke

1. SymbolDetail の `ストラテジー / 検証結果` から `この銘柄でストラテジー提案` を開く。
2. StrategyLab 上部で symbol code、market、timeframe、return path の context banner が表示されることを確認する。
3. local_llm / stub の通常 proposal は、symbol_code / market / timeframe を使う軽い context proposal として扱う。銘柄固有の最新情報や Web 検索済みの説明を期待しない。
4. Codex CLI section が `Codex CLIで銘柄調査付き候補JSONを作成する` に切り替わり、通常の Web検索 checkbox が表示されないことを確認する。
5. `銘柄調査付きCodex promptを作成` を押し、prompt が Codex CLI 側の Web検索を前提に、対象銘柄の基本情報、価格傾向、出来高、volatility、決算・業績、news、events、sector 環境の確認を要求することを確認する。
6. prompt は `strategy_proposal_candidates` v1.0 JSON object 1 個だけを要求し、URL / citation / 長い引用、`source_type=web`、raw Web result を含めないことを確認する。
7. JSON import 後に候補を選択しても、title / natural language spec へ反映するだけで、Pine generation、save、backtest、AI summary、自動 application 作成が起動しないことを確認する。
...
38. targeted repair template がある issue では、repair prompt が `repair_template` を `repair_hint` より優先し、2 回目以降の repair attempt では recurring note により同一 issue の再発修正を強めることを確認する。

## 投資カレンダー walkthrough

Home 上部で `投資カレンダー` section を確認し、監視・保有銘柄の予定と市場イベントが 1ヶ月分の compact calendar grid で表示されることを確認する。`前月` / `次月` で表示月だけが切り替わり、Home 全体の再取得や provider refresh が起きないことを確認する。event がない日付 cell は日付1行程度に圧縮され、event がある日だけ chip 量に応じて高さが広がること、今日を含む月では該当 day cell に `今日` badge と控えめな強調が出ることを確認する。日付 cell には title 中心の短い event chip が積まれ、provider / source / fetched_at / importance text などの補助 metadata が通常表示の chip 内に出ないこと、chip hover / focus で sanitized detail card が出ること、同日に event が多い場合は `+N件` として省略されること、土日 cell が平日と控えめに区別されること、provider / fetched_at / stale / partial_success の情報が grid 下部にまとまることを確認する。必要に応じて `投資カレンダーを更新` を押し、失敗時も Home 全体が壊れないことを確認する。

SymbolDetail では `銘柄概要` の直下に `投資カレンダー` section があり、TradingView chart widget は表示されない。投資カレンダーは Home と同じ 1ヶ月 compact calendar grid で表示され、対象銘柄に関係する event が day cell に積まれることを確認する。`前月` / `次月` は frontend local state の表示切替だけで、URL 変更、API fetch、provider refresh を起こさない。今日 badge、土日区別、空 cell 圧縮、event chip、`+N件`、hover / focus detail、grid 下部の provider / fetched_at / stale metadata、`カレンダーを更新` button、0 件補足を確認する。TradingView CSV import と BacktestDetail への導線は従来どおり確認する。
...
- StrategyVersionDetail の `内部バックテスト` section で、symbol context がある場合は Symbol UUID が prefill され、通常表示では symbol id / code を入力できることを確認する。`内部バックテストを実行` を明示クリックした場合だけ internal backtest API が呼ばれること、成功時は metrics summary と `検証レポートを開く` link が表示されることを確認する。表示だけでは internal backtest、Pine generation、AI summary、optimization、application apply は起動しない。
