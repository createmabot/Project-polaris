...
- Visual regression は導入せず、browser smoke と unit test を優先する。導入検討時は seed data、timestamp / locale、state fixture、viewport、CI runtime を先に安定させる。
...
## 33. Visual regression readiness 確認

1. Visual regression / screenshot comparison は今回導入しないことを確認する。
2. 新規 Playwright spec、package script、snapshot、CI required check が追加されていないことを確認する。
3. 初回 pilot 候補は 1〜3 画面に絞り、`ApplicationDetail` / `SymbolDetail` を優先する方針を確認する。
4. dynamic timestamp / locale、seed順序、raw JSON、long page、外部 / AI 由来表示、viewport、browser、CI runtime は mask または対象外の検討対象であることを確認する。
...
4. ApplicationDetail の report履歴は入口であり、AI summary / artifact 詳細は BacktestDetail で確認する説明を確認する。
5. BacktestComparisonDetail は保存済み pairwise comparison の再訪画面であり、AI summary 同士の比較や artifact diff は後続判断である説明を確認する。
6. 確認では API / backend / DB / Prisma schema / artifact 操作 / AI summary 自動生成を変更しない。
...
5. AI summary 自動生成、AI summary 同士の比較、artifact file read / download、artifact diff / JSON diff、Visual regression pilot が後続判断として残っていることを確認する。
...
1. CSV import report auto enqueue は direct CSV import route と application 起点 CSV import route の `parse_status=parsed` 直後を対象にすることを確認する。
2. internal backtest report conversion auto enqueue は、新規 internal_backtest Backtest report が作成された直後だけを対象にすることを確認する。
3. 既存 report を返す idempotent conversion、BacktestDetail 初回表示、ApplicationDetail report history 表示、batch / scheduled job 起点では enqueue しないことを確認する。
4. 同一 `inputSnapshotHash` の succeeded summary、queued / running job、failed job がある場合は自動 enqueue せず、failed job の自動 retry は行わないことを確認する。
5. 既存 manual generation endpoint / button、CSV import、internal backtest 実行、report conversion UI、DB schema は維持する。
6. AI summary 同士の比較、artifact diff、artifact download、polling 本格化は後続判断として残す。
