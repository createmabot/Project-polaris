...
- 銘柄起点ストラテジー適用フローの正本は docs/48.北極星 銘柄起点ストラテジー適用フロー設計（P3）.md とする。
- `SymbolDetail` から `ストラテジー / 検証結果` 領域を起点に、CSV 取込、内部バックテスト、検証レポート詳細、銘柄別比較へ接続する設計であり、実装前の導線確認観点も docs/48 を参照する。
