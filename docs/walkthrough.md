...
## 33. PR本文の文字コード方針（2026-05-09 追加）

- PR 本文は文字化け回避のため ASCII English のみを使う。
- 日本語の詳細説明は docs に記録し、PR 本文は要約とカテゴリ名だけに留める。
- PR 本文には日本語ファイル名や `G:\Projects\...` のようなローカル Windows path を含めない。
- `.github/pull_request_template.md` を既定テンプレートとし、`Summary` `Files changed` `Scope` `Checks` `Not included` `Secret check` の形式を使う。
- docs 本文は UTF-8 日本語で保存してよく、PR 本文は正本ではなく補助要約として扱う。
