...
- 対応 CSV 形式や import 制約は [docs/34.北極星 TradingView CSV import 運用手順（MVP）.md](G:\Projects\hokkyokusei\docs\34.北極星 TradingView CSV import 運用手順（MVP）.md) を正本とし、設計上も矛盾させない。
...
- `SymbolDetail` に `ストラテジー / 検証結果` 空 section を追加する
- `StrategyList` / `StrategyDetail` の画面設計を先に固める
