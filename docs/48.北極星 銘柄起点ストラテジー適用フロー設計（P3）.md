...
- `SymbolDetail` が将来読む application list API として `GET /api/symbols/:symbolId/strategy-applications` を追加した。
- UI 接続は未実装であり、現在の `SymbolDetail` apply UI は selection-only のままである。
- POST application API、CSV取込、内部バックテスト接続は後続で扱う。

## 追記（2026-05-10 その8）

- `SymbolDetail` apply UI の保存先として、`POST /api/symbols/:symbolId/strategy-applications` を追加した。
- UI からの保存接続はまだ未実装であり、現在の `SymbolDetail` apply UI は selection-only のままである。
- CSV取込、内部バックテスト接続、BacktestReport 作成は後続タスクとして残す。

## 追記（2026-05-10 その9）

- `SymbolDetail` apply UI を POST API に接続し、選択中 strategy / version を Symbol Strategy Application として保存できるようにした。
- 保存済み application list を `SymbolDetail` の `ストラテジー / 検証結果` section に表示する。
- CSV取込 / 内部バックテストは後続タスクとして disabled のまま維持する。

## 追記（2026-05-10 その10）

- `SymbolDetail` から保存済み application 起点で TradingView CSV を取り込めるようにした。
- CSV取込は Backtest / BacktestImport / SymbolStrategyApplicationRun を作成し、BacktestDetail は report detail として維持する。
- internal backtest wiring、BacktestDetail backlink、related reports / applied symbols display は後続タスクとして残す。

## 追記（2026-05-10 その11）

- `SymbolDetail` から保存済み application 起点で internal backtest を開始できるようにした。
- internal backtest は InternalBacktestExecution と SymbolStrategyApplicationRun を作成し、run から execution へ link する。
- Backtest / BacktestImport は作成せず、BacktestDetail report 化、BacktestDetail backlink、result detail 表示は後続タスクとして残す。

## 追記（2026-05-10 その12）

- 銘柄起点 application / run / report の関係を `StrategyDetail` / `SymbolDetail` / `BacktestDetail` から read-only に確認できるようにした。
- `StrategyDetail` では適用済み銘柄と関連検証レポート、`BacktestDetail` では銘柄起点 application backlink を表示する。
- report detail としての `BacktestDetail` の役割は維持し、archive / restore や internal execution result detail は後続タスクに残す。

## 追記（2026-05-10 その13）

- `SymbolDetail` / `StrategyDetail` から Symbol Strategy Application を archive / restore できるようになった。
- archived application は履歴保持のため削除せず、runs / reports は引き続き参照できる。
- hard delete、internal execution result detail、BacktestDetail redesign は未実装である。

## 追記（2026-05-10 その10）

- `SymbolDetail` の保存済み application row で、latest run が `internal_backtest` の場合に internal backtest execution の status / result summary を read-only 表示するようにした。
- 表示は既存 internal backtest read API を利用し、Backtest / BacktestImport 作成や `BacktestDetail` report 化は行っていない。
- internal execution result の詳細画面化や report backlink refinement は後続タスクとして残す。

## 追記（2026-05-10 その12）

- BacktestDetail の銘柄起点 backlink 表示を read-only で整理した。
- Symbol Strategy Application / Application Run / Backtest report の関係を BacktestDetail から確認できるようにし、SymbolDetail / StrategyDetail / StrategyVersionDetail への参照導線を維持した。
- BacktestDetail は引き続き個別検証レポート詳細であり、application parent には置き換えない。
