...
- CSV import report 系では、`parseStatus=parsed` かつ `parsedSummaryJson` 保存後に Backtest AI summary generation を自動起動する。状態保存は既存 `ai_jobs` / `ai_summaries` を利用し、schema 追加は行わない。
