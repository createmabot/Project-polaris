...
- `report_source`、strategy / version filter、run type / run status filter、application-specific runs endpoint は後続判断とする。
...
- `report_source`、strategy / strategy version、run type、run status による絞り込みは未実装として残す。
