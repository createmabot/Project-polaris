...
- CSV import report 系では、direct Backtest import と application 起点 `POST /api/symbol-strategy-applications/:applicationId/csv-import` の両方で、`parseStatus=parsed` かつ `parsedSummaryJson` 保存後に Backtest AI summary generation を自動起動する。状態保存は既存 `ai_jobs` / `ai_summaries` を利用し、schema 追加は行わない。
- parse failed import は保存するが、自動起動対象にはしない。
...
## 追記（2026-05-12）Application Detail / History API・UI現在地

- application-specific runs endpoint と reports endpoint は read-only history API として実装済み。
- Application Detail は `run履歴` / `report履歴` を application 単位で表示し、filter / pagination を既存 query で扱う。
- `SymbolDetail` は application 概要、latest run、latest report、CSV / internal latest pair、Application Detail 入口に責務を絞る。
- Application Detail では metrics 欠損値説明と read-only browser smoke まで完了済み。
- DB migration / Prisma schema change / backend API shape 変更は行っていない。
- comparison entity、metrics normalization、artifact diff、AI summary 比較は後続判断とする。

## 追記（2026-05-13）Symbol Strategy Application / Backtest report 比較の現在地

- application-specific runs / reports endpoint は read-only history API として維持する。
- BacktestDetail は個別 report detail を主語にし、同一 application 内の current / related report metrics を近接比較補助として表示する。
- CSV import report の metrics は BacktestImport parsed summary 由来、internal backtest report の metrics は strategySnapshotJson.result_summary 由来として扱う。
- ApplicationDetail は application 単位の run / report 履歴探索、BacktestComparisonDetail は保存済み pairwise comparison / 本格比較画面候補として責務を分ける。
- comparison entity、metrics normalization、artifact diff、AI summary 自動比較は後続判断とし、DB migration / Prisma schema change / backend API shape 変更は行わない。

## 追記（2026-05-13）Application Detail report履歴からの比較補助入口

- Application Detail の report履歴は read-only history API の表示に留める。
- 各 report row の BacktestDetail link は、同一 application 内の関連 report と metrics を確認する comparison helper 入口として扱う。
- この入口は既存 `backtest_detail_link` を使うだけで、新規 comparison entity、API shape 変更、report conversion / CSV import / internal backtest 実行は行わない。

## 追記（2026-05-13）Report comparison UX phase 2 完了時点の現在地

- Symbol Strategy Application / Run / Backtest report の関係は、CSV import report と internal backtest report を同一 application 内で read-only に近接比較補助できる段階まで整理済みである。
- `BacktestDetail` は個別 report detail と current / related report metrics comparison helper を担当し、CSV import report は `BacktestImport.parsedSummaryJson`、internal backtest report は `Backtest.strategySnapshotJson.result_summary` を metrics root とする。
- `ApplicationDetail` は application 単位の run / report history、`BacktestComparisonDetail` は保存済み pairwise comparison の再訪画面として責務を分ける。
- `-` は metrics root に該当 metric がない意味であり、comparison entity や normalization table の未作成とは別に説明する。
- comparison entity、metrics normalization table、artifact diff / JSON diff、AI summary 自動比較、BacktestComparisonDetail 本格化は後続判断とする。
- DB migration、Prisma schema change、backend API shape change、新規 comparison entity、実行系操作は行っていない。

## 追記（2026-05-13）AI summary / artifact 現在地

- CSV import report の AI summary input は `BacktestImport`、parsed summary、comparison diff、TradingView 実行結果文脈を主に使う。
- internal backtest report の AI summary input は `Backtest.strategySnapshotJson.result_summary`、`artifact_pointer`、`internal_backtest_execution_id`、importless report 文脈を主に使う。
- `artifact_pointer` は internal backtest artifact の参照 metadata であり、file 実体読込 / download / diff の契約ではない。
- `BacktestDetail` は個別 report の AI summary / artifact pointer / raw artifact JSON 表示を担当し、`ApplicationDetail` は report history の探索入口に留める。
- `BacktestComparisonDetail` は保存済み pairwise comparison の AI summary 表示を担当し、AI summary 同士の比較や artifact diff は後続判断とする。
- この整理は docs-only であり、API、backend、DB migration、Prisma schema は変更しない。

## 追記（2026-05-13）AI summary / artifact operations phase completion 現在地

- AI summary / artifact operations phase は docs/53 の完了整理を正本として、PR #309〜#310 の範囲で完了扱いにする。
- Symbol Strategy Application / Run / Backtest report の関係は維持し、CSV import report と internal backtest report の source 差は `BacktestDetail` の説明で扱う。
- `ApplicationDetail` は run / report history の入口であり、AI summary / artifact の詳細確認は `BacktestDetail` に送る。
- `BacktestComparisonDetail` は保存済み pairwise comparison の AI summary 表示を担当し、AI summary 同士の自動比較や artifact diff は後続判断とする。
- API shape、backend、DB migration、Prisma schema change は行わない。

## 追記（2026-05-13）AI summary 自動生成運用設計の現在地

- AI summary 自動生成の運用設計は docs/56 を正本として扱う。
- CSV import report は `BacktestImport` / parsed summary / comparison diff / TradingView 文脈、internal backtest report は `strategySnapshotJson.result_summary` / `artifact_pointer` / `internal_backtest_execution_id` / importless report 文脈を主 input とする。
- 初回自動生成候補は report 作成直後の enqueue 1 系統に限定し、CSV import parsed report 作成直後または internal backtest report conversion 完了直後のどちらかで後続判断する。
- 現時点では API / backend / DB migration / Prisma schema change は行わず、既存 `ai_jobs` / `ai_summaries` / provider boundary と手動生成 endpoint を維持する。

## 追記（2026-05-13）AI summary auto enqueue phase 1 API現在地

- CSV import report auto enqueue は、direct CSV import route と application 起点 CSV import route の `parse_status=parsed` 直後を phase 1 完了範囲にする。
- internal backtest report conversion auto enqueue は、application 起点 report conversion で新規 internal_backtest Backtest report が作成された直後だけを first implementation 範囲にする。
- 既存 report を返す idempotent conversion、guarded update 競合で既存 report を返す conversion、BacktestDetail / ApplicationDetail 表示起点、batch / scheduled job 起点は対象外にする。
- 状態と生成物は既存 `ai_jobs` / `ai_summaries` を使い、DB migration / Prisma schema change は行わない。
- 同一 `inputSnapshotHash` の succeeded summary、queued / running job、failed job がある場合は自動 enqueue しない。failed job の retry は既存 manual generation endpoint / button に委ねる。
- internal backtest report は `BacktestImport` を作らない importless report として扱い、`strategySnapshotJson.result_summary`、`artifact_pointer`、`internal_backtest_execution_id` を Backtest AI summary input に含める。
