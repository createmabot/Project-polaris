...
- `report_source` は後続で最小実装済み。strategy / version filter、run type / run status filter、application-specific runs endpoint は後続判断とする。
...
- `report_source` は後続で最小実装済み。strategy / strategy version、run type、run status による絞り込みは未実装として残す。
