...
- confidence calibration: confidence は利益期待ではなく、ルール明確性、Pine feasible、uncertainty の低さとして読めるかを見る。backtest 前 / Web search なし / 最新 market data なしの候補で high が多すぎる場合は品質課題として扱う。
...
## 7-5. Timeframe canonicalization smoke

2026-05-23 の Strategy proposal timeframe smoke では、stub provider で `JP_STOCK` / `US_STOCK` と `D` / `4H` / `1H` の 6 組み合わせを確認した。

確認結果:

- `D` は swing / trend following / breakout、volume / moving average / RSI、サンプル期間・決算 / イベント・gap の caution を含む前提として確認する。
- `4H` は short swing / momentum / pullback / breakout confirmation、false breakout、overnight / session gap、liquidity、短めの invalidation を含む前提として確認する。
- `1H` は intraday / short-term momentum / mean reversion / volatility breakout、whipsaw、transaction cost、slippage、overtrading、tighter stop / shorter holding を含む前提として確認する。
- `timeframe_assumption` は canonical `D` / `4H` / `1H` と一致する。
- `1D` は UI 選択肢に出さず、backend request parsing / history filter では `D` に正規化する。
- Codex CLI prompt と Web検索付き prompt option でも timeframe guidance は維持する。
- proposal selection は title / natural language spec への反映だけで止め、Pine generation / save / backtest / AI summary は自動起動しない。

記録上の注意:

- smoke 結果には raw prompt、raw provider response、raw Codex output、candidate free text full dump、endpoint、model 実値、secret、local path、stack trace を残さない。
- real local_llm / real Codex CLI は manual optional のまま扱い、required check に入れない。
