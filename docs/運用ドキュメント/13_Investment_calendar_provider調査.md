...
- 実装難易度: 高。
- 北極星への採用候補: P3 以降の optional paid provider。初回採用は見送り。
- 備考: 会社 event の網羅性は高いが、今回の manual refresh / 個人運用には cost が大きい。
...
- freshness: 高。
- source reliability: 高。
- 利用規約上の懸念: scraping は避ける。初回は J-Quants または seed / curated fixture を優先。
- 実装難易度: 低から中。
...
- 実装難易度: 低。
- 北極星への採用候補: P1 の economic indicator / IPO / US earnings 補助候補。
...
- 実装難易度: 中。
- 北極星への採用候補: P1 の macro event enrichment / release metadata 候補。
...
- 実装難易度: 中。
- 北極星への採用候補: P1 の market-level provider 候補。API key / plan が許容できる場合に採用。
- 備考: Home の market-level event を広く実用化するなら最も直接的。
...
- 実装難易度: 低から中。
- 北極星への採用候補: P1/P2 の代替 provider 候補。Alpha Vantage / J-Quants で不足する場合に比較する。
- 備考: economic calendar と holidays を 1 provider に寄せやすい。
...
- 実装難易度: 中。
- 北極星への採用候補: 初回は見送り。FMP / Alpha Vantage / Trading Economics との比較候補。
- 備考: 既存 community 情報だけでは品質判断しない。採用前に current pricing / endpoint access を確認する。
...
- 実装難易度: 中。
- 北極星への採用候補: P1 の curated official source。初回は official page 由来の seed / manual curated fixture でも可。
...
- 利用規約上の懸念: HTML parsing の安定性。
- 実装難易度: 中。
- 北極星への採用候補: P1 の market-level provider 候補。
...
- 利用規約上の懸念: HTML parsing の安定性。
- 実装難易度: 低から中。
- 北極星への採用候補: P1 の market holiday source。初回は curated fixture / manual refresh provider の対象。
...
- source: Trading Economics または FMP を optional API provider とし、FOMC / BOJ / exchange holidays は official source / curated fixture と組み合わせる。
- symbol-level 日本株 event は J-Quants 採用判断まで stub / seed 維持。
- manual refresh only。
...
- source: J-Quants を第一候補、TDnet API は paid optional、EDINET は filing enrichment に限定。
- market-level event は FOMC / BOJ / holiday 程度に抑える。
- 長所: 日本株ユーザーの銘柄別確認価値が高い。
- 短所: J-Quants plan / credential / 利用条件確認が先に必要。TDnet API は費用が重い。

### Option C: 経済指標重視構成

- 米 CPI、雇用統計、FOMC、日銀、休場日を最優先する。
- source: Trading Economics / FMP / Alpha Vantage / official central bank pages。
...
- J-Quants / TDnet は日本株 symbol-level では有力だが、credential、plan、再配布条件、coverage 確認を先に行う必要がある。
...
- source: Trading Economics または FMP を optional provider とし、FOMC / BOJ / holiday は official source / curated fixture の hybrid を検討する。
- 実装:
...
- 対象: TDnet 適時開示、EDINET filing。
- source: TDnet API は paid optional、EDINET は submitted document event。
- 実装:
