...
- AI summary auto-generation phase 1 は CSV import parsed report、application 起点 CSV import parsed report の auto enqueue までを現行完了範囲とする。internal backtest report conversion endpoint は Stage 2C cleanup で削除済みのため、新規 auto enqueue は発生しない。
...
- Artifact 表示は BacktestDetail の artifact pointer metadata read-only 表示に限定する。Stage 2C cleanup 後、internal backtest artifact read endpoint / table は存在せず、download、signed URL、file token、diff の入口ではない。
...
- AI summary auto enqueue は CSV import parsed report、application 起点 CSV import parsed report 直後に限定する。新規 internal backtest report conversion 起点は Stage 2C cleanup 後存在しない。
