...
local_llm Pine generation の timeout guard は `PINE_GENERATION_LOCAL_LLM_TIMEOUT_MS` で制御する。既定は 180 秒、上限は 300 秒とし、Strategy proposal の `STRATEGY_PROPOSAL_LOCAL_LLM_TIMEOUT_MS` とは別に調整する。設定変更後は backend process を再起動する。timeout を延ばしても TradingView compile、自動 backtest、AI summary 自動連鎖は有効化しない。
...
- `STRATEGY_PROPOSAL_LOCAL_LLM_TIMEOUT_PROFILE` は default / long_context を扱う。default は通常の local model でも遅い応答に 180 秒程度まで余裕を持たせた profile、long_context はさらに重い local model や長文 context model の manual smoke 向けに 240 秒程度まで待つ明示 opt-in。
