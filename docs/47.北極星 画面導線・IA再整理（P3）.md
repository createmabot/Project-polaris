...
現行 route は `frontend/src/App.tsx` を正とし、次の画面で構成される。
...
- [docs/45](./45.北極星%20browser-based%20E2E導入方針（P3）.md) の browser-based E2E 方針を維持する
...
- docs/47 で整理した IA の代表導線として、Home → SymbolDetail → StrategyDetail → BacktestDetail の read-only browser smoke を追加した。
- SideRail から銘柄詳細に入り、保存済み application を経由して strategy と検証レポートの参照導線を確認する。
- CRUD、CSV import、internal backtest、archive / restore などの実行導線は scenario に含めない。

## 追記（2026-05-10 その10）

- `SymbolDetail` の銘柄起点 strategy application 表示を小さく整理した。
- StrategyDetail / StrategyVersionDetail / BacktestDetail への参照導線は維持し、IA 上の役割は変更していない。
- CSV import / internal backtest / archive restore の実行仕様はそのままとする。

## 追記（2026-05-10）

- `BacktestDetail` の銘柄起点 backlink 表示を整理した。
- `SymbolDetail` / `StrategyDetail` / `StrategyVersionDetail` へ戻る導線は維持し、検証レポート詳細としての役割は変更していない。

## 追記（2026-05-10）

- P3 の画面導線・IA の現在地は [[53.北極星 P3現在地と残課題整理（P3）]] に整理した。
- Home → SymbolDetail → StrategyDetail → BacktestDetail の read-only 導線は smoke として固定済みで、CRUD / import / execution 実行を含む scenario は後続判断とする。

## 追記（2026-05-13）画面導線 / IA debt cleanup

- Header / global navigation から `/watchlist` と `/positions` への直接リンクを外した。
- 主要ナビゲーションは Home、Strategy、StrategyLab、Backtest report の主要導線に絞る。
- `/watchlist` と `/positions` route は削除せず、移行期の補助 route として残す。
- 監視銘柄 / 保有銘柄の一覧移動、追加、編集、削除は引き続き SideRail を主要導線とする。
- SideRail 内の旧管理画面リンクは `詳細管理` の補助導線として扱い、主要操作ではないことを文言で補足する。
- WatchlistManage / PositionsManage には、主要操作が SideRail に移行済みである説明を追加する。
- API、backend、DB、Prisma schema、SideRail refresh optimization、latest home data sharing、watchlist / positions refresh split は変更しない。
