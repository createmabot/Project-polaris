...
- LLM strategy proposal 設計: `docs/仕様書/11_LLM_Strategy_Proposal仕様.md`
...
- セットアップ後チェック: [`docs/運用ドキュメント/02_起動と確認手順.md`](docs/%E9%81%8B%E7%94%A8%E3%83%89%E3%82%AD%E3%83%A5%E3%83%A1%E3%83%B3%E3%83%88/02_%E8%B5%B7%E5%8B%95%E3%81%A8%E7%A2%BA%E8%AA%8D%E6%89%8B%E9%A0%86.md) と [`docs/walkthrough.md`](docs/walkthrough.md)
...
- `GET /api/backtests/:backtestId` は optional read-only field `latest_ai_summary_job` で最新 job status（`queued|running|succeeded|failed`）も返す。
- CSV import が `parse_status=parsed` になった場合は、CSV import report 系に限り Backtest AI総評生成を自動起動する。
- succeeded internal backtest execution が新規 Backtest report に変換された場合も、report conversion 直後に Backtest AI総評生成を自動起動する。既存 report を返す idempotent path では起動しない。
- 同一 input の succeeded summary、queued / running job、failed job がある場合、自動起動は重複作成や自動 retry を行わない。手動生成 API / button は維持する。
- BacktestDetail は latest job status を read-only 表示するが、polling / live update は行わない。必要時は手動再読み込みで確認する。
...
- SideRail の監視銘柄 / 保有銘柄追加は `symbol_code` だけでも登録できます。既存 Symbol があれば name / market / tradingview 情報を利用し、未登録の4桁数字は `JP_STOCK` / `TSE:<symbol_code>` の既存慣例で補完します。
...
- The proposal flow is for verification candidates, not investment advice. Proposal history, provider observation, optional benchmark, sanitized benchmark summary records, and sanitized provider quality trend are available; Web search, deep research, and automatic Pine generation remain future decisions.
- `STRATEGY_PROPOSAL_PROVIDER=local_llm` is opt-in. local_llm schema failures are handled with strict JSON prompting, safe JSON extraction, lightweight normalization, and sanitized provider status only.
...
- Visual regression / screenshot comparison is introduced as an optional pilot only.
- Run it with `pnpm test:e2e:visual`; it is not included in `pnpm test:e2e:browser` and is not a CI required check.
- The pilot target is limited to the `ApplicationDetail` `application summary` stable container.
- Dynamic timestamp / locale output is masked. Raw JSON, long pages, external rendering, AI-derived text, and TradingView widget output stay out of scope.
- Update the baseline only after intentional UI changes with `pnpm --filter frontend test:e2e:visual -- --update-snapshots`, then review the generated image diff.
...
- 次候補は StrategyVersionDetail、StrategyLab、SymbolDetail filter の慎重な部分適用、補助画面整理、Visual regression 本導入可否の再判断です。

### AI summary auto-generation operations design

- AI summary 自動生成の運用設計は `docs/56.北極星 AI summary 自動生成運用設計（次フェーズ）.md` を正本とする。
- AI summary auto-generation phase 1 は PR #319 / #320 / #332 により、direct CSV import route、application 起点 CSV import route、internal backtest report conversion の auto enqueue まで完了扱い。
