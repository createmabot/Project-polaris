          ...
          messages: [
            {
              ...
              content: [
                ...
                'Candidate diversity: make candidates meaningfully different. Do not return the same MA/RSI idea with only threshold changes. Vary strategy_type, entry trigger, exit rule, risk control, and market regime assumption.',
                'risk_preference alignment: reflect input.risk_preference in risk_management, invalidation_conditions, backtest_cautions, confidence, and uncertainty. Conservative candidates should use stronger confirmation, lower trading frequency, and lower position risk. Aggressive candidates may use faster triggers but must include stronger drawdown, slippage, and overfitting cautions.',
                'strategy_type_bias alignment: if input.strategy_type_bias is not any, make at least the first candidate follow that bias. If it is any, diversify strategy_type across candidates.',
                'suggested_natural_language_spec quality: include market, timeframe, long/short assumption, entry trigger, exit trigger, stop loss rule, indicator periods, and backtest caution. Convert vague phrases such as momentum or overheat into measurable conditions whenever possible.',
                'confidence calibration: before backtest, Web search, or latest market data, use confidence=high sparingly. confidence means rule clarity, Pine feasibility, and lower uncertainty, not expected profit.',
                'research_basis usage: explicit user_hint conditions are user_input; market, timeframe, and risk settings are internal; common strategy archetypes are provider_knowledge. Do not use source_type=web or URL citations.',
            ...
            ...(retryContext
              ? [{
                ...
                content: JSON.stringify({
                  ...
                  required_behavior: [
                    'Include every required candidate key in every candidate object.',
                    'Every scalar field must be a non-empty string. Use only allowed enum strings for strategy_type, pine_feasibility, and confidence.',
                    'Use non-empty arrays of strings for entry_logic, exit_logic, risk_management, invalidation_conditions, expected_strengths, expected_weaknesses, required_indicators, backtest_cautions, uncertainty, and suggested_pine_constraints.',
                    'research_basis must be a non-empty array of objects with source_type, label, and url. source_type must be internal, user_input, or provider_knowledge.',
                    'suggested_natural_language_spec must be a concrete Japanese rule description of at least 20 characters.',
